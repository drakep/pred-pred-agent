...
Prob = .0001 #Probability point will be populated
    ...
    for j in range(0,len(A)):
        ...
        if A[j][1] > Size:
            ...
            if A[j][1] < 0:
                ...
                A[j][1] = A[j][1]
        if (A[j][2] < 20 and A[j][2] != 0): #Grow Up Counter, max 'grow' size, grow larger by eating
            A[j][2] = A[j][2] + random.random()*.001
        if A[j][4] >= 5000:
            A[j][2] = 0
        if A[j][4] < 5000: # Starve to Death Counter
            A[j][4] = A[j][4] + 1
    return A
...
scalefix.append(points(pos=[Size,Size],color=color.black))
scalefix.append(points(pos=[0,0],color=color.black))
scalefix.append(points(pos=[0,Size],color=color.black))
scalefix.append(points(pos=[Size,0],color=color.black))
B = []
C = []
countmulti = 0
LiveOrDie = 0
